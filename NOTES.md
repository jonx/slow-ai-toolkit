...
### v0.4.0 - docs-restructure: structure is a separate discipline from truth

Distilled from two real cleanups of agent-maintained repositories. The symptom was the same in both: not lies, but accretion - status stated in six places, no Markdown links between documents, a 1,500-line procedure file, a 950-line handover mixing role, status and abandoned ideas, tools with no index. `docs-refresh` could not fix that: it verifies and rewrites *words*, and a true README beside six stale status copies is still a mess. The fix is structural and mostly mechanical, so it became its own skill rather than a heavier refresh: measure first; move and split by script so links survive; one home per fact (state / design / procedure / history); finished-state prose everywhere except the journal; a rule set for the next agent and a checker (`check-docs.py`) that fails on broken links, stale generated tables of contents, dangling identifier references, missing navigation blocks and missing index rows. Two deliberate choices: hand-rewrite only the top-level documents and leave technical bodies to a convert-as-touched rule (an agent rewriting 3,000 lines it cannot verify is the other failure mode), and name the pair as siblings - `docs-refresh` makes the words true, `docs-restructure` makes the tree navigable; restructure first, then refresh. A merged "docs" skill was rejected: two triggers, two protocols, one would always be loaded for the wrong reason.
